...
local steam_http_raw = ffi.cast("uint32_t**", ffi.cast("char**", ffi.cast("char*", client.find_signature("client.dll", "\xB9\xCC\xCC\xCC\xCC\xE8\xCC\xCC\xCC\xCC\x83\x3D\xCC\xCC\xCC\xCC\xCC\x0F\x84")) + 1)[0] + 48)[0] or error("steam_http error")
...
function http.new(task)
	task = task or {}
	local instance = setmetatable({requests = {}, task_interval = task.task_interval or 0.3, enable_debug = task.debug or false, timeout = task.timeout or 10}, http_mt)
...
function http:_process_tasks()
	...
	client.delay_call(self.task_interval, http._bind(self, '_process_tasks'))
end
